  ...
  return (
    <section className={HOME_BAND_CLASS} aria-labelledby="explore-heading">
      <div className="mx-auto max-w-6xl">
        ...
        <ul className="mt-8 grid list-none gap-3 sm:mt-10 sm:grid-cols-2 sm:gap-4 lg:grid-cols-4">
          {PRIMARY_NAV.map((item, i) => (
            <li key={item.href}>
              <ScrollReveal className="h-full" delayMs={120 + i * 65}>
                <Link
                  href={item.href}
                  className="flex min-h-[7.5rem] h-full flex-col justify-between rounded-2xl border border-blue-100/90 bg-surface p-5 shadow-md shadow-blue-900/[0.06] outline-none ring-accent/25 transition-all duration-300 ease-out active:scale-[0.99] hover:-translate-y-0.5 hover:border-amber-300/90 hover:shadow-lg hover:shadow-amber-900/[0.08] focus-visible:ring-2 sm:min-h-0 sm:p-6"
                >
                  <span className="h-1.5 w-10 rounded-full bg-gradient-to-r from-blue-500 to-amber-400" />
